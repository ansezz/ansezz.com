...
tags: [messaging, cloud-platforms, laravel, scaling, architecture]
featured: false
heroImage:
...
This is the heart of the [shift from synchronous to asynchronous communication](/blog/synchronous-vs-asynchronous-communication/). When a user signs up on your site, you publish a `UserSignedUp` event to a topic. Your main app is done. It returns a success message to the user immediately. Meanwhile, various subscribers pick up that event and do their jobs in the background.
...
![Dashboard mockup comparing pull and push Pub/Sub subscription delivery metrics side by side](/blog/event-driven-pubsub/dashboard-mockup.webp)
...
![Pop-art illustration of a dead-letter queue catching failed poison messages routed off the main pipeline](/blog/event-driven-pubsub/dlq-illustration.webp)
...
For those of us in the PHP and Laravel ecosystem, integrating Google Pub/Sub is smooth. Laravel ships with solid queue support for Redis and SQS, but the `google/cloud-pubsub` client lets you tap into GCP's global scale. This kind of decoupling is exactly what makes the [move from a monolith to microservices](/blog/monolith-to-microservices/) tractable instead of terrifying.
