...
tags: [mcp, api, ai, llm, agentic-ai, architecture]
featured: false
heroImage:
...
Using standard APIs, you would need to feed the descriptions of all those endpoints into the model's context window. This eats up tokens and increases the risk of the model getting confused. With MCP, the agent only sees the tools it needs at the moment it needs them. The protocol handles the context streaming, ensuring that the model has the right data without being overwhelmed by irrelevant metadata.

![An AI agent dashboard dynamically selecting tools from an MCP server](/blog/api-vs-mcp/agent-tool-selection.webp)

Furthermore, MCP allows for "resource" access. Unlike an API endpoint that just returns data, an MCP resource can be a living document or a continuous stream of information. This is crucial for RAG (retrieval-augmented generation) systems. Instead of building a complex ETL pipeline to move data into a vector database, an MCP server can expose your local files or database directly to the model in a standardized way. This avoids the common [7 RAG mistakes in production](/blog/7-rag-mistakes-production/).
...
![A side-by-side code comparison of JSON REST and MCP tool definitions](/blog/api-vs-mcp/rest-vs-mcp-code.webp)
