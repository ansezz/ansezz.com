...
1. **Enforce them mechanically.** Human discipline fails under deadline. Add an architecture test that fails the build when `Modules\Billing` imports `Modules\Inventory\Models`. In PHP, [Deptrac](https://github.com/opensoftwareconsulting/deptrac) or a Pest arch test does this in a few lines. Any language has an equivalent.
