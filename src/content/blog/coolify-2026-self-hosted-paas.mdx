...
description: "Coolify crossed 60k GitHub stars and 280+ one-click services. What that actually buys you, how it compares to Dokploy, CapRover and Dokku on real numbers, and the security claims you should not believe."
publishDate: 2026-08-09
category: devops
tags: [coolify, self-hosting, devops, docker, paas, deployment, dokploy, vps]
featured: false
heroImage:
...
As of today the repo sits at **60,298 GitHub stars**. It ships **280+ one-click services**. Version 4 has been the stable line for a while now. It is no longer a hobby project you're brave for running — it's infrastructure with a bus factor above one.
...
- **Portability.** Moving from a $5 droplet to a dedicated Hetzner box is a server swap, not a migration project.
...
The catalog spans roughly:

- **Databases**: PostgreSQL, MySQL, MariaDB, MongoDB, Redis, ClickHouse, and vector stores like Qdrant.
- **Dev tools**: Gitea, Hoppscotch, n8n, Uptime Kuma.
- **Analytics**: Plausible, Umami, PostHog.
- **CMS and apps**: Ghost, Strapi, Directus, WordPress, Appwrite, Supabase.
...
| **Build sources**   | Dockerfile / Compose / Nixpacks / static | Dockerfile / Compose / Nixpacks / Heroku buildpacks | Dockerfile / captain-definition | Herokuish buildpacks / Dockerfile |
...
**Dokploy** is the real competitor now, not a footnote — 36k stars and climbing fast, with a template catalog that has quietly grown _larger_ than Coolify's. It's leaner and the UI is arguably cleaner. If you're starting fresh in 2026 it deserves a genuine evaluation rather than a dismissal. Coolify's remaining edge is time in production: more people have hit more edge cases, and those fixes are already merged.
...
**CapRover** is stable and Swarm-native, but development has slowed relative to the others. Fine if you're already on it; I wouldn't start there.
...
- **Automatic TLS.** Let's Encrypt issuance and renewal via the bundled proxy (Traefik by default, Caddy optional), including wildcard certificates with a DNS challenge.
- **Encrypted secrets.** Environment variables are encrypted at rest, scoped per-resource, with shared variables at the project and team level.
- **Team roles.** Owner, admin, and member roles per team, so a contractor doesn't get production database access.
- **Scheduled S3 backups.** Database dumps to any S3-compatible bucket — Backblaze B2 and Cloudflare R2 both work and cost almost nothing.
...
The math is not subtle. Ten small projects on a managed platform — a few hobby-tier databases, some build minutes, a bit of bandwidth — lands somewhere around $150–250/month once you're past free tiers. The same ten projects on a €20 Hetzner box with 8 vCPU and 16GB of RAM run comfortably, with headroom, for the price of two coffees.
...
- **Put the control plane on its own box.** A €5 server running only Coolify. It's the cheapest insurance in this entire stack.
