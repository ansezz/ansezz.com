...
I hit this wall on a multi-tenant SaaS build last year. The agent produced migrations, models, policies, jobs, and tests in one pass. The diff was correct — mostly. I spent two evenings reading it line by line, approved it, and then found the actual bug in production three weeks later: a queued job that ran outside the tenant scope. I had read that file. I had read that exact method. My eyes went over it and my brain filed it as "looks like the other ones."

That is the honest failure mode. Reading every line felt like diligence. It was theatre.
...
That last one is the real trick. Every review comment I would type twice becomes a rule instead. If the agent can violate an invariant and the pipeline stays green, that is my bug, not the agent's.
...
One giant "review this PR" prompt produces mush. Narrow agents with narrow briefs produce findings you can act on. The setup I run now has three, and they all execute inside CI rather than in my editor:
...
Two rules keep this from becoming noise. First, findings are severity-ranked and anything below "would break in production" goes to a comment, never a block. Second, a second agent tries to _refute_ each finding before I see it. Roughly a third of the initial findings die there, and they are the ones that would have burned my afternoon.
