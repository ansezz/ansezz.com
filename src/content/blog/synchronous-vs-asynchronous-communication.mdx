...
This is more like sending an email. You hit send and immediately go back to your day. The recipient reads and processes the message whenever they are available. In a technical stack, this is achieved using message queues like Redis or Amazon SQS, or a dedicated broker such as [RabbitMQ](/blog/scaling-with-rabbitmq/).
...
For cross-service communication, Google Cloud Tasks is a strong alternative to a self-hosted queue. It dispatches HTTP callbacks to any endpoint with built-in rate limiting, scheduling, and retries. This is particularly useful when building an [API gateway for AI stacks](/blog/api-gateway-ai-stack/), where different microservices need to talk to each other without blocking the main user flow.
