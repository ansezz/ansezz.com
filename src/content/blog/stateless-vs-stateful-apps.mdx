---
title: "Stateless vs stateful apps: choosing the right architecture"
description: "Stateful servers remember; stateless servers forget by design. Why horizontal scaling, sticky sessions, and shared Redis decide the split — with Laravel Sanctum and Shopify examples."
publishDate: 2026-06-22
category: architecture
tags: [architecture, scalability, laravel, redis, devops, shopify]
featured: false
heroImage:
...
A stateless application is like a goldfish with a very efficient filing cabinet. It treats every single HTTP request as a completely new event. The server does not remember anything about the client once the request is finished. If a user clicks a button, the server processes the data, sends a response, and immediately forgets that user ever existed.
...
The primary advantage of this approach is horizontal scalability. Since the server does not store any local session data, any server in your cluster can handle any request. You can add ten more servers to your stack using a tool like [Coolify or Docker](/blog/coolify-docker-saas-hosting/) and the load balancer can distribute traffic without worrying about where the user's session is located. This is the foundation of modern [API gateway architectures](/blog/api-gateway-ai-stack/).
...
In the world of PHP development, Laravel gives you the flexibility to choose either path. By default, the `web` middleware group uses stateful sessions. It stores a session cookie on the browser and matches it to a file or database entry. This is perfect for traditional dashboards where you need high security and simple state management.

To make your Laravel app stateless, you typically move toward token-based authentication using Laravel Sanctum or Passport. You would disable session cookies for your API routes and ensure that all user data is retrieved from a shared store like Redis on every request. This shift is essential when building backends for mobile apps or complex React/Vue frontends.

When your app grows, you might even implement advanced patterns like [circuit breakers with vector databases](/blog/circuit-breakers-vector-db/) to ensure that your stateless services remain performant even when external data sources are under heavy load.
...
Stateful apps require much more care. If you are deploying a stateful Laravel app, you must ensure that your session driver is set to something central like Redis or a database. If you leave it set to "file," your horizontal scaling will fail. You also need to consider how you handle deployments. A "zero-downtime" deployment is harder when you have thousands of active sessions living in server memory. You often have to wait for sessions to drain before taking a node offline.
