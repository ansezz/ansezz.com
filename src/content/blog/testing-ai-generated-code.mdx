...
description: "AI-generated code is 95% syntactically clean and fails security tests 45% of the time. Half of agentic PRs ship no tests. The verification pipeline I run instead."
...
One genuinely encouraging data point: reasoning-focused models with extended thinking hit 70–72% pass rates, the best observed. That is real improvement. It is still a coin flip you would never accept from a human contributor.
...
A July 2026 empirical study accepted at ICSME analysed 4,882 agent-generated pull requests from the AIDev dataset — 532 Java, 4,350 Python, across five coding agents. Two findings should reshape how you gate merges:

**Agents included test changes in only 49.6% of PRs that modified code covered by test files.** Not "PRs with no test infrastructure available" — PRs where tests for that code already existed and the agent changed the code without touching them.

**Existing test suites did not pick up the slack.** They executed 61.5% of the agents' changed executable lines in Java and just 27.0% in Python. In 64.8% of Python PRs, _not one changed line_ was executed by any existing test. The safety net people assume is there is mostly hole.
...
Google's 2025 DORA report on AI-assisted development is the macro version of everything above. Its central finding: higher AI adoption correlates with an increase in delivery throughput _and_ an increase in delivery instability, at the same time.

That is not an argument against AI. It is a statement about where the bottleneck moved. When change volume goes up and the control systems — automated testing, version control discipline, fast feedback — stay where they were, instability is the arithmetic result. DORA's own ROI model shows how thin the margin gets: a change failure rate drifting from 5% to 6% wipes out a meaningful chunk of the productivity gain in downtime cost alone.

AI amplifies whatever your delivery process already was. This is the same trade-off I mapped out in [AI vs traditional development](/blog/ai-vs-traditional-development/), now with three years of industry data behind it.
...
jobs:
  ...
  mutation:
    ...
    steps:
      ...
      - run: vendor/bin/infection --git-diff-filter=AM --min-msi=70 --threads=max
```
